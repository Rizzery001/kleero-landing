...
export default function PrivacyPage({ params: { locale } }: Props) {
  ...
  const filePath = path.join(process.cwd(), 'src/content/legal', `privacy-${lang}.md`);
  ...
  return (
    <main className="min-h-screen bg-cream-50 py-20">
      <div className="container-x max-w-3xl mx-auto">
        <article
          className="prose prose-stone prose-headings:font-display prose-headings:text-forest-700 prose-a:text-terracotta-500 max-w-none"
        >
...
function MarkdownContent({ source }: { source: string }) {
  // simple markdown rendering: split by lines and render headings + paragraphs + tables
            ...
            <table key={i}>
              ...
              <tbody>
                {block.rows.map((row, j) => (
                  <tr key={j}>
                    {row.map((cell, k) => (
                      <td key={k} dangerouslySetInnerHTML={{ __html: inlineFormat(cell) }} />
                    ))}
                  </tr>
                ))}
              </tbody>

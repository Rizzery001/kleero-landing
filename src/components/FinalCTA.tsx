...
export default function FinalCTA() {
  ...
  const primaryLabel = FEATURES.SHOW_WHATSAPP ? t('ctaTelegram') : t('ctaPrimary');
        ...
        <div className="relative overflow-hidden rounded-2xl bg-forest-700 text-cream-50 px-6 sm:px-10 lg:px-16 py-14 lg:py-20 text-center">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              {FEATURES.SHOW_WHATSAPP && (
                <a href={whatsappLink('landing_cta')} target="_blank" rel="noopener noreferrer" className="btn-primary">💬  {t('ctaWhatsapp')}</a>
              )}
              <a href={telegramLink('landing_cta')} target="_blank" rel="noopener noreferrer" className={FEATURES.SHOW_WHATSAPP ? "inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 bg-cream-50 text-forest-700 font-medium tracking-tight text-[15px] transition-all duration-200 hover:-translate-y-0.5 shadow-[0_2px_8px_rgba(0,0,0,0.15)]" : "btn-primary"}>✈️  {primaryLabel}</a>
            </div>
          </div>
        </div>
      </div>
